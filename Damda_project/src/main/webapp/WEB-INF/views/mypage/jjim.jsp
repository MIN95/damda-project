...
function price_setting() {
    ...
    for (var i = 0; i < length; i++) {
       var id = stocks[i].split('_');
       console.log('itemNum : '+id[1]);
...
function cartAdd(no){
    ...
    console.log('itemArr id배열 : '+itemArr);
    ...
    for (var i = 0; i < length; i++) {
     ...
     if(check == true){
		...
		$('#compareModal').modal('show');
	     	...
	     	$.ajax({
				url:"/mypage/insertcart?itemnum="+no,
				type:'post',
				data:"",
...
function deleteItem(itemNum){
    $.ajax({
		url:"/mypage/wishdelete?itemnum="+itemNum,
		type:'post',
		data:"",
		success: function(data){
        	console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>success');
    ...
    if ($('input:checkbox[name="chk"]').is(":checked")) {
        ...
        for (var i = 0; i < length; i++) {
            if(stocks[i] > 0){
	            ...
	            $.ajax({
	            	url:'/mypage/wishdelete?itemnum='+stocks[i],
	        		type:'post',
	        		data:"",
	        		success: function(data){
	                	console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>success');
...
function compareCart() {
    ...
    if(itemArr.length>0){
		...
		console.log('장바구니 아이템넘버: '+cartItem);
	    ...
	    console.log('위시리스트 중복 제거: '+stocks);
    ...
    for(var i =0; i<stocks.length; i++){
    	$.ajax({
        	url:'/mypage/insertcart?itemnum='+stocks[i],
    		type:'post',
    		data:"",
    		success: function(data){
            	console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>success');
		...
		<input type="hidden" id="itemNum${cartList.c_itemNum }" value="${cartList.c_itemNum }" class="itemNum">
             ...
             <div id="content_content" class="col-md-6">
                ...
                <table>
                    ...
                    <tbody>
                    	<c:forEach items="${List }" var="List" varStatus="status">
	                        <tr>
	                            <td>
	                                <input type="checkbox" name="chk" id="${List.w_itemNum }" />
	                                <label for="${List.w_itemNum }"></label>
	                            </td>
	                            <td>
	                                <a href="/item/detail?no=${List.w_itemNum }"><img src="/resources/imgs/${List.photoCtg }/${List.photoName}" /></a>
	                            </td>
	                            <td>
	                                <a href="/item/detail?no=${List.w_itemNum }"><input type="text" name="itemname" id="itemname${List.w_itemNum }" value="${List.itemName }" readonly /></a>
	                                <p class="itemprice_${List.w_itemNum }">원</p>
	                                <input type="hidden" class="itemprice" id="itemprice_${List.w_itemNum }" value="${List.itemPrice }" readonly />
	                            </td>
	                            <td>
	                                <button type="button" id="cartAdd${List.itemNum }" onclick="cartAdd(${List.itemNum });">장바구니 담기</button>
	                                <button type="button" id="deleteItem${List.itemNum }" onclick="deleteItem(${List.itemNum });">삭제</button>
	                            </td>
	                        </tr>
                    	</c:forEach>
                    </tbody>
                ...
                <div class="col-md-offset-3 col-md-9 col-sm-offset-4 col-sm-6 col-xs-offset-2 col-xs-8">
				    <ul class="pagination">
				        <li><a href = "/mypage/jjim?p=0" aria-label="Previous">&laquo;</a></li>
				        ...
				        <c:otherwise>
				        <li><a href = "/mypage/jjim?p=${p-1 }">‹</a></li>	        
				        </c:otherwise>
				        </c:choose>
				        
				        <!-- 해당하는 페이지로 갈 수 있는 버튼 -->
				        <c:forEach var="i" begin="0" end="${totalpage}">
				        <li><a href = "/mypage/jjim?p=${i }">${i+1 }</a></li>
				        </c:forEach>
				        ...
				        <li><a href = "/mypage/jjim?p=${totalpage }" aria-label="Next">&raquo;</a></li>
				    </ul>
                </div>
