				  ...
				  <nav aria-label="Page navigation">
					<ul class="pagination">
					  ...
					  <c:if test="${map.noticePager.curPage < map.noticePager.totPage}">
					  	<li><a href="javascript:list('${map.noticePager.curPage + 1 }','${map.noticePager.totPage }')" aria-label="Previous">〉</a></li>
					  </c:if>
				    </ul>
			      </nav>
				</div>
 	...
 	$(document).ready(function(){
		...
		//글 삭제시 전체 선택&해제(관리자)
 		...
 		$('#deleteBtn').click(function(){
 			//삭제 체크 안되어있을 시 모달 text변경
 			var checkbox = $("input[name=deleteCheck]:checked");
 			if($("input[type='checkbox']").is(':checked')==false){
 				$('#confirmModal>div>div>div:nth-child(1)>h4').text("선택된 항목이 없습니다");
 				$('#confirmModal>div>div>div:nth-child(2)>button:nth-child(1)').css('display','none');
 				$('#confirmModal>div>div>div:nth-child(2)>button:nth-child(2)').text("확인");
 			}else{
 				$('#confirmModal>div>div>div:nth-child(1)>h4').text("선택한 항목을 삭제하시겠습니까?");
 				$('#confirmModal>div>div>div:nth-child(2)>button:nth-child(1)').css('display','');
 				$('#confirmModal>div>div>div:nth-child(2)>button:nth-child(2)').text("취소");
 	 		}
 			//모달 띄우기
	  	    ...
	  	    $("#confirmOk").click(function(){
		  	    ...
		  	    $.ajax({
					...
					success : function(result){
						location.href="/notice/?curPage=${map.noticePager.curPage}";
					},
